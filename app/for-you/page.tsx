...
function SelectedBookSkeleton() {
  return (
    <div className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] items-center gap-6 bg-[#fbefd6] rounded-lg p-4 sm:p-6 animate-pulse">
      <div className="space-y-3">
        <div className="h-4 w-full rounded bg-[#d8e3e0]" />
        <div className="h-4 w-4/5 rounded bg-[#d8e3e0]" />
      </div>
      <div className="flex items-center gap-4">
        <div className="aspect-square w-1/3 max-w-[140px] shrink-0 rounded bg-[#d8e3e0]" />
        <div className="space-y-3 flex-1 min-w-0">
          <div className="h-4 w-4/5 rounded bg-[#d8e3e0]" />
          <div className="h-3 w-3/5 rounded bg-[#d8e3e0]" />
          <div className="h-3 w-2/5 rounded bg-[#d8e3e0]" />
        </div>
      </div>
    </div>
  );
...
export default function ForYouPage() {
  ...
  const [selectedBookDuration, setSelectedBookDuration] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    const onLoaded = () => {
      ...
      const mins = Math.floor(total / 60);
      const secs = total % 60;
      setSelectedBookDuration(
        `${mins} mins ${secs.toString().padStart(2, "0")} secs`,
      );
    };
    audio.addEventListener("loadedmetadata", onLoaded);
  ...
  return (
    <>
      <div className="max-w-[1070px] mx-auto pt-8 pb-0 px-4 md:px-0">
        <section>
          ...
          {isLoading ? (
            ...
            selectedBook && (
              <div
                className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] items-center gap-6 bg-[#fbefd6] rounded-lg p-4 sm:p-6 cursor-pointer"
                onClick={() => router.push(`/book/${selectedBook.id}`)}
              >
                <p className="text-[#394547] font-medium text-sm sm:text-base">
                ...
                <div className="flex items-center gap-4">
                  <Image
                    ...
                    width={140}
                    height={140}
                    className="rounded w-1/3 max-w-[140px] h-auto shrink-0 object-cover"
                  />
      ...
      <div className="max-w-[1070px] mx-auto pb-8 space-y-16 px-4 md:px-0 overflow-hidden mt-16">
        <section>
          <h2 className="text-[22px] font-bold text-[#394547] mb-1">
            Recommended For You
          </h2>
          <p className="text-base text-[#394547] mb-4">
            ...
            recommendedBooks.length > 0 && (
              <div className="grid grid-flow-col auto-cols-[140px] sm:auto-cols-[170px] md:auto-cols-[200px] gap-3 sm:gap-4 overflow-x-auto snap-x snap-mandatory scrollbar-hide pb-2">
                {recommendedBooks.map((book) => (
                  <div
                    key={book.id}
                    className="relative snap-start p-3 pt-8 rounded-2xl overflow-hidden transition-colors hover:bg-[#f0f4f3] cursor-pointer"
